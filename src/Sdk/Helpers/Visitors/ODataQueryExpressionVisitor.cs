namespace Microsoft.Pfe.Xrm.Sdk
{
    ...
    public class ODataQueryExpressionVisitor : IQueryExpressionVisitor
    {
        ...
        public string QueryString => string.Join("&", _queryOptions.Select(x => $"{x.Key}={WebUtility.UrlEncode(x.Value)}"));
        ...
        private ColumnSet VisitColumnSet(ColumnSet columnSet)
        {
            ...
            {
                var columns = columnSet.Columns.Distinct();
        ...
        private IEnumerable<OrderExpression> VisitOrderExpressions(IEnumerable<OrderExpression> orders)
        {
            if (orders != null && orders.Count() > 0)
            {
                ...
                foreach (var order in orders)
                {
                    ...
                    orderBys.Add($"{order.AttributeName} {direction}");
                }
        ...
        private int? VisitTopCount(int? topCount)
        {
            if (topCount != null && topCount > 0)
            {
                _queryOptions.Add("$top", topCount.ToString());
            }
            return topCount;
        }
        ...
        {
            var attribute = expression.AttributeName;
